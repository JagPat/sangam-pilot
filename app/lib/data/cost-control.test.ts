...
describe('Cost Control data mapping',()=>{
  it('maps official totals and workflow rows without inventing family funding',()=>{
    const result=mapCostControlWedding(
      ...
      [{id:'e1',wedding_id:'w1',cost_item_id:'i1',version_number:1,total:118000,subtotal:100000,tax_rate:18,currency_code:'INR',state:'approved',scope_included:null,scope_excluded:null,remarks:null,decision_due_at:null,created_by_account_id:'a1'}],
      [],[],[],
...
describe('Cost Control draft editing',()=>{
  it('keeps editable draft fields when mapping an estimate',()=>{
    const result=mapCostControlWedding(
      ...
      [{id:'e1',wedding_id:'w1',cost_item_id:'i1',version_number:1,total:118000,subtotal:100000,tax_rate:18,currency_code:'INR',state:'draft',scope_included:'Stage floral installation',scope_excluded:'Lighting',remarks:'Awaiting vendor reply',decision_due_at:'2026-08-01T00:00:00Z',created_by_account_id:'a1'}],
      [],[],[],[],
      'a1',
    );
    expect(result.items[0].estimates[0]).toMatchObject({
      id:'e1',scopeIncluded:'Stage floral installation',scopeExcluded:'Lighting',remarks:'Awaiting vendor reply',decisionDue:'2026-08-01T00:00:00Z',canEditDraft:true,
    });
  });

  it('withholds draft edit permission from another Cost Control viewer',()=>{
    const result=mapCostControlWedding(
      {id:'w1',title:'A & B'},['cost_approver'],
      [{id:'c1',wedding_id:'w1',name:'Decor',sort_order:10}],
      [{id:'i1',wedding_id:'w1',cost_centre_id:'c1',title:'Stage',description:null,lifecycle_state:'planning',decision_due_at:null}],
      [{id:'e1',wedding_id:'w1',cost_item_id:'i1',version_number:1,total:118000,subtotal:100000,tax_rate:18,currency_code:'INR',state:'draft',scope_included:null,scope_excluded:null,remarks:null,decision_due_at:null,created_by_account_id:'a1'}],
      [],[],[],[],
      'a2',
    );
    expect(result.items[0].estimates[0].canEditDraft).toBe(false);
  });
  ...
  it('gates draft edit and submit controls on creator permission',()=>{
    const page=readFileSync(resolve(process.cwd(),'app/host/cost-control/page.tsx'),'utf8');
    expect(page).toContain("e.state==='draft'&&e.canEditDraft");
  });
