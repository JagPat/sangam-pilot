...
// wedding owner (full), or a writable family admin scoped to one side (host_group). Co-host is explicitly
// view-only and therefore never produces adminGroupId. Used by the guest screen so a
...
export type OperatorContext = {
  ids: string[]; // wedding ids where I hold any operator role, including view-only co-host
...
export async function getOperatorContext(db: AppSupabaseClient): Promise<OperatorContext> {
  ...
  for (const r of data ?? []) {
    ...
    else if (r.role === 'host_group_admin' && r.host_group_id) cur.adminGroupId = cur.adminGroupId ?? r.host_group_id;
