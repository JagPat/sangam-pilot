...
type Estimate={id:string;wedding_id:string;cost_item_id:string;version_number:number;total:number;subtotal:number;tax_rate:number;currency_code:string;state:string;remarks:string|null};
...
export type CostControlEstimate={id:string;version:number;subtotal:number;taxRate:number;total:number;currency:string;state:string;remarks:string|null};
...
export function mapCostControlWedding(
  ...
  return {
    ...
    items:items.filter((i)=>i.wedding_id===wedding.id).map((i)=>({
      ...
      estimates:estimates.filter((e)=>e.wedding_id===wedding.id&&e.cost_item_id===i.id).map((e)=>({id:e.id,version:e.version_number,subtotal:Number(e.subtotal),taxRate:Number(e.tax_rate),total:Number(e.total),currency:e.currency_code,state:e.state,remarks:e.remarks})).sort((a,b)=>b.version-a.version),
...
export async function getCostControl(db:AppSupabaseClient):Promise<CostControlWedding[]>{
  ...
  const [weddings,centres,items,estimates,commitments,invoices,payments,summary]=await Promise.all([
    ...
    app.from('cost_estimate_version').select('id,wedding_id,cost_item_id,version_number,total,subtotal,tax_rate,currency_code,state,remarks').in('wedding_id',ids),
