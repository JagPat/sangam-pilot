...
type Estimate={id:string;wedding_id:string;cost_item_id:string;version_number:number;total:number;subtotal:number;tax_rate:number;currency_code:string;state:string;scope_included:string|null;scope_excluded:string|null;remarks:string|null;decision_due_at:string|null};
...
export type CostControlEstimate={id:string;version:number;subtotal:number;taxRate:number;total:number;currency:string;state:string;scopeIncluded:string|null;scopeExcluded:string|null;remarks:string|null;decisionDue:string|null};
...
export function mapCostControlWedding(
  ...
  invoices:Invoice[],payments:Payment[],summary:Summary[],
):CostControlWedding{
...
export async function getCostControl(db:AppSupabaseClient):Promise<CostControlWedding[]>{
  ...
  const [weddings,centres,items,estimates,commitments,invoices,payments,summary]=await Promise.all([
    ...
    app.from('cost_estimate_version').select('id,wedding_id,cost_item_id,version_number,total,subtotal,tax_rate,currency_code,state,scope_included,scope_excluded,remarks,decision_due_at').in('wedding_id',ids),
  ...
  return (weddings.data??[]).map((w)=>mapCostControlWedding(w,relevant.filter((r)=>r.wedding_id===w.id).map((r)=>r.role),
    centres.data??[],items.data??[],estimates.data??[],commitments.data??[],invoices.data??[],payments.data??[],summary.data??[]));
}
