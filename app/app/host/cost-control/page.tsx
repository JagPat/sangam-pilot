...
function DecisionForm({w,id,kind}:{w:string;id:string;kind:'estimate'|'commitment'}){
  const action=kind==='estimate'?decideEstimate:decideCommitment;
  return <form action={action} className="sg-formrow"><Hidden w={w}/><input type="hidden" name={kind==='estimate'?'estimateId':'commitmentId'} value={id}/>
    <select className="sg-select" name="decision"><option value="approved">Approve</option>{kind==='estimate'?<option value="revision_required">Request revision</option>:null}<option value="rejected">Reject</option></select>
