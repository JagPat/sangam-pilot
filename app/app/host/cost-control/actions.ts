...
export async function decideEstimate(fd:FormData){await command('decide_cost_estimate',{p_wedding:text(fd,'weddingId'),p_estimate:text(fd,'estimateId'),p_decision:text(fd,'decision'),p_reason:text(fd,'reason'),p_expected_state:'under_review'});}
export async function proposeCommitment(fd:FormData){await command('propose_cost_commitment',{p_wedding:text(fd,'weddingId'),p_item:text(fd,'itemId'),p_estimate:text(fd,'estimateId'),p_engagement:null,p_quote_reference:optional(fd,'reference'),p_commitment_date:optional(fd,'commitmentDate')});}
export async function decideCommitment(fd:FormData){await command('decide_cost_commitment',{p_wedding:text(fd,'weddingId'),p_commitment:text(fd,'commitmentId'),p_decision:text(fd,'decision'),p_reason:text(fd,'reason')});}
...
export async function verifyInvoice(fd:FormData){await command('verify_cost_invoice',{p_wedding:text(fd,'weddingId'),p_invoice:text(fd,'invoiceId'),p_reason:text(fd,'reason')});}
...
export async function voidPayment(fd:FormData){await command('void_cost_payment',{p_wedding:text(fd,'weddingId'),p_payment:text(fd,'paymentId'),p_reason:text(fd,'reason')});}
