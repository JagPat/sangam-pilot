...
// owner_create_event / owner_update_event RPCs, which own the zoned_time composite + offset math and the
// is_wedding_owner check. Everything runs as the signed-in user; no service role.
...
export async function addVenue(fd: FormData): Promise<void> {
  ...
  try {
    ...
    const { error } = await app.from('venue').insert({
      wedding_id: weddingId, name, iana_timezone: tz || 'Asia/Kolkata', address: address || null,
    });
    if (error) throw error;
  } catch (e) {
...
// carries the full current field set, so the RPC never wipes a field it wasn't meant to.
